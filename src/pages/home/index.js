...
function Home(){
    ...
    useEffect(()=>{
        async function loadFilmes(){
            ...
            setFilmes(response.data.results.slice(0,30)) //limita a quantidade da lista de filmes
            setLoading(false);
        }
        loadFilmes();
    },[])
                    ...
                    {Filmes.map((filme) =>{
                        return(
                            <article key={filme.id}>
                                ...
                                <Link to ={`/filme/${filme.id}`}>DETALHES</Link> {/*link para acessar detalhes do filme */}
                            </article>
                        )
                    })}
                </div>
